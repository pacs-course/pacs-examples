...
LinearAlgebra::MyMat0<Real, LinearAlgebra::ROWMAJOR> Hilbert(unsigned int n);

int main()
{
  using namespace LinearAlgebra;
  // I create two matrices
  MyMat0<Real, COLUMNMAJOR> a;
  //MyMat0 a;
  a.resize(3,4);
  MyMat0<Real> b(5,5);
  ...
  std::vector<Real> va(4,1.0);
  std::vector<Real> res(a*va);
  std::cout<<"result of a*[";
  for (std::vector<Real>::iterator it=va.begin();it<va.end()-1;
       ++it)std::cout<<*it<<",";
  std::cout<<*(va.end()-1)<<"]^T =";
  std::cout<<"[";
  for (std::vector<Real>::iterator i=res.begin();i<res.end()-1;++i)std::cout<<*i<<", ";
  std::cout<<*(res.end()-1)<<"]"<<std::endl;

  std::cout<< "Now testing copy constructors and move semantic"<<std::endl;
  MyMat0<Real, ROWMAJOR> t(a);
  std::cout<<" This matrix should be equal to a"<<std::endl;
  t.showMe();
  MyMat0<Real, ROWMAJOR> tc(std::move(t));
...
LinearAlgebra::MyMat0<Real, LinearAlgebra::ROWMAJOR> Hilbert(unsigned int n)
{
  LinearAlgebra::MyMat0<Real, LinearAlgebra::ROWMAJOR> tmp(n,n);
