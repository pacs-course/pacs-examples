 ...
 *
 * This implementation allows to treat both real and complex polynomials, moreover, if the arguments
 * are constant expressions the compiler may compute the result statically
 *
 * @tparam Coeff The type of the container with the coefficients: either std::array or std::vector
 ...
 * @return The evaluation of the polynomial. The type is the common type between coefficients and point x
...
constexpr auto HornerConstexpr(Coeff const & coeff,Value const & x)
{
  ...
  std::common_type_t<decltype(coeff.back()),Value> u = coeff.back();// back is a constexpr function for arrays
