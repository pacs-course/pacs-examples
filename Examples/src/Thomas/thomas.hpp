...
namespace apsc
{
  ...
  /*!
    ...
    b[0] and c[n-1] are not used. a,b and c can be any stl compliant sequential container

    @note a, b and c MUST be of the dimension of the system even is some element are not used
    <b>The dimension of the system is determined by the size of a</b>.

    @tparam C type of a stl compliant sequential container. Typically a vector<double>
  ...
  //! Solution of tridiagonal system on a system of n equations and symmetry conditions
   /*!
     System is of the form
...
which represent a problem with jump conditions x_{n-1}-x_0=f_{n-1}
     ...
     @note a, b and c MUST be of the dimension of the system even is some element are not used
    <b>The dimension of the system is determined by the size of a: n=a.size()</b>
  ...
  C thomasSolveSym(const C & a, const C& b, const C & c, const C & f)
  {
    ...
    // reduced system, I need to reduce only a, since it governs the size of the reduces system
...
/*!
 ...
 @note   <b>The dimension of the system is determined by the size of a: n=a.size()</b> 
*/
  
  template<class C>
  C matVecTrid(const C & a, const C& b, const C & c, const C & v, const bool sym=false)
  {
    using size_t = decltype(a.size());
     auto n = a.size();
     C res;
