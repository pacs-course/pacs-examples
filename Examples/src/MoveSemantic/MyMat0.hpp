 ...
 */
template <StoragePolicySwitch T = ROWMAJOR> class MyMat0
{
private:
  size_type nr, nc;
  //! Data storage
  ...
  /*!
    ...
    The advantage with respect to the solution with a pointer to function is
    that now the method can be inlined, with greater efficiency if it is called
    many times (as it normally would be...).
   */
  inline size_type getIndex(size_type const, size_type const) const;
...
public:
  ...
  */
   ...
   */
  template <StoragePolicySwitch S> MyMat0(MyMat0<S> const &m);
#ifndef NOMOVE
#warning "Move constructor active"
  ...
  */
  template <StoragePolicySwitch S> MyMat0<T> &operator=(MyMat0<S> const &m);
#ifndef NOMOVE
#warning "Move copyassignement active"
  ...
  */
   ...
   */
  void vecMultiply(const std::vector<double> &v,
...
{
template <StoragePolicySwitch T>
MyMat0<T>::MyMat0(size_type n, size_type m) : nr(n), nc(m), data(n * m, 0.0)
{}
...
// General case (different storage order since otherwise the non-templated
...
template <>
size_type
MyMat0<ROWMAJOR>::getIndex(size_type const i, size_type const j) const
{
  return j + i * nc;
}
...
template <>
size_type
MyMat0<COLUMNMAJOR>::getIndex(size_type const i, size_type const j) const
{
  return i + j * nr;
}
...
template <StoragePolicySwitch T>
double
MyMat0<T>::getValue(size_type const i, size_type const j) const
{
...
template <StoragePolicySwitch T>
void
MyMat0<T>::setValue(size_type const i, size_type const j, double const &v)
{
  if(i < 0 || i >= nr || j < 0 || i <= nc)
    throw std::out_of_range(" Out of bounds in MyMat0");
  this->operator()(i, j) = v;
...
template <StoragePolicySwitch T>
void
MyMat0<T>::resize(size_type const n, size_type const m)
{
  if(n * m != nc * nr)
    {
      // clear data storage
  ...
  for(size_type i = 0; i < nr; ++i)
    {
      double vsum = 0;
      for(size_type j = 0; j < nc; ++j)
        vsum += std::abs(this->operator()(i, j));
...
template <>
void
MyMat0<COLUMNMAJOR>::vecMultiply(const std::vector<double> &v,
                                 std::vector<double> &      res) const
{
  if(v.size() != nc)
    {
...
template <>
void
MyMat0<ROWMAJOR>::vecMultiply(const std::vector<double> &v,
                              std::vector<double> &      res) const
{
  if(v.size() != nc)
    {
...
template <StoragePolicySwitch T>
void
MyMat0<T>::fillRandom(unsigned int seed)
{
...
template <StoragePolicySwitch T>
void
MyMat0<T>::showMe(std::ostream &out) const
{
  out << "[";
  for(size_type i = 0; i < nr; ++i)
    {
...
template <StoragePolicySwitch T>
std::vector<double>
operator*(MyMat0<T> const &m, std::vector<double> const &v)
{
