...
template <std::size_t M> struct metaDot
{
  ...
  static constexpr T
  apply(std::array<T, N> const &a, std::array<T, N> const &b)
  {
...
/*!
 * A different implementation (since C++17). More flexible: it treats also
 * complex numbers!
...
constexpr auto
metadot17(std::array<T1, N> const &a, std::array<T2, N> const &b) noexcept
{
