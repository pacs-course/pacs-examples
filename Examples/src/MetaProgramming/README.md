...
Some of the examples can be simplified with c++20 concepts. They are still a useful set of example to learn "the dark side of C++".
...
* *for_each_tuple* How to run a callable object on the members of a tuple. The pre c++20 version show also an example of use of `std::integer_sequence`, superseded, in c++20, by `std::apply()`;
...
* *IsClonable.* An example to show how to check if a class has a particular method, in this case the method `clone()`. Now C++17 has introduced the type trait `std::void_t` which can be used instead of my handcrafted one. You can also check if `clone()` returns the right return type, but it is too involved so I decided to omit the test. The complete solution is in `Utilities/CloningUtilities.hpp`.
...
* *SwitchType*	We show first how types stored in a class can be used to determine template derived types. A mesh class selects automatically 
	...
	the intrinsic dimension is mapped to the corresponding element.
...
* *transposeView* An example of a view. A view is a special proxy class that allows to give a different semantic to an object. In this case, we have a class 
	...
	specific Matrix, 2) the use of `std::is_const<Matrix>` to verify if the stored matrix is const (in this case I cannot change the elements). This version has been updated to c++20.
	
* *Trivial*. This example shows the difference and some application of `std::is_trivial`, `std::is_trivially_constuctible` and `std::has_standard_layout`. If an object is trivially constuctible can be serialized (sufficient condition!) if it has standard layout it may be converted to pointer to the first member or passed to a C++/fortran function as a normal struct (in C++) or a structure type in Fortran. Again, it is just a sufficient  condition.

# What do I learn here? #

-Some examples of template metaprogramming.