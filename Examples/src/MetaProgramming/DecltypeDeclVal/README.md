...
The `double` argument of `sizeof` is obviously not a value! It means that the argument of `size_of` is in an `unevaluated context`, the value is irrelevant and not used, that's why we can just write `double` and we do not need to provide a value. Note that this is completely different than an ordinary function
...
Even if I do `sizeof(3.0)`, the *value* '3.0' is not used, the compiler needs to know that it is a double. Again, I can also do `sizeof(double())`, by calling the constructor of a double explicitly: no double is constructed, since I am in an *unevaluated context*.
...
decltype(double) a;
```
is valid, though useless since `a` is now a double and I could have simply written `double a`.
...
I want to automatically deduce the type returned by `A::f(double)` using `decltype` (extracting the type returned by a member function may be useful in generic programming)
...
Fine, but what the role of `declval`? Well, the problem is that in a generic setting the previous statement has a problem. The class `A` (which is a generic program may depend on a template parameter) may not have a default constructor! In this case the statement fails, because even if `A()` is not evaluated, it must be well formed.

We can get around this problem by using the `declval<>` magic. How it works is very technical, but the use is relatively simple. `declval<A>()` is an expression
that can be used **only in an unevaluated context** and returns "something" (in fact a rvalue reference) that allows to do the trick:
