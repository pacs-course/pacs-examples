...
namespace TypeTraits
{
  ...
  */
  ...
  /*!  Primary template for has_clone.  
    ...
    @tparam Sfinae A dummy template parameter used only for activating SFINAE. It should have a default  value
    but it is indifferent which one, void is fine. You don't need to name the parameter since it is not used
    in the body of the class. We could have written
     @code
  ...
  /*!  Specialized version that is activated if T is clonable.
    ...
    version is discarded. If not, this specialized version will be used because the second template parameter
    is now given, and specialized version have precedence. Note that it inherits from std::true_type, so it is
    convertible to true.
    
    declval<T&>() allows to test the return type of clone() with no need of creating an object of type T.
    Moreover, we use T& because T may be a polymorphic object, and clone() may be defined in the base class.

    @note I am not checking the return type of clone() only its existence It is possible also to check the return type,
    but it becomes more involved. I might to use std::enable_if<>. For instance to check whether clone() returns
