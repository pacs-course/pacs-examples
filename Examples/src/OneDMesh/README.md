...
a variable mesh spacing. The latter needs to integrate an ordinary differential equation to locate the mesh nodes; to this purpose it uses one of the  runge kutta adaptive schemes
provided in the directory `RKFSolver/`

`main_TestGenerator` is a simple application to test the software and `run_test_generator.sh` is a script that runs the code and launches `gnuplot`.
