...
namespace Geometry{
  ...
  std::ostream& operator << (std::ostream & out, Point2D const & a)
  {
  ...
  void AbstractPolygon::setVertexes(Vertices const & v)
  {
    vertexes=v;
    checkConvexity();
  }
  ...
  bool AbstractPolygon::checkConvexity()
  {
    //! for simplicity
  ...
  */
  ...
  void Square::checkSquare()
  {
    ...
    double constexpr
      smallNumber=100*std::numeric_limits<double>::epsilon();
  ...
  void Square::setVertexes(Vertices const & v){
    this->vertexes=v;
    checkSquare();
  }
  ...
  void Triangle::setVertexes(Vertices const & v){
    this->vertexes=v;
    checkTriangle();
  }
