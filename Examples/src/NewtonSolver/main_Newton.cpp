...
main()
{
  ...
  */
  ...
  auto nonLinFun = [](const ArgumentType &x) -> ArgumentType {
    ...
    y(1) =
      x[0] * x[0] - 81 * (x[1] + 0.1) * (x[1] + 0.1) + std::sin(x[2]) + 1.06;
...
*/
  ...
  // Open a scope so that the structured bindings object is eventually deleted
  {
    ...
    std::cout << std::boolalpha
              << "Newton Solution has converged: " << converged
              << " last iteration:" << iter << " last residual:" << resNorm
    ...
    apsc::make_Jacobian(apsc::JacobianKind::DiscreteJacobian));
  {
    ...
    std::cout << std::boolalpha
              << "Newton Solution with discrete jacobian has converged: "
              << converged << " last iteration:" << iter
              << " last residual:" << resNorm << " has stagnated:" << stagnated
              << std::endl;
  }
    ...
    apsc::make_Jacobian(apsc::JacobianKind::BroydenB));
  {
    ...
    std::cout << std::boolalpha
              << "Broyden (bad) Solution has converged: " << converged
              << " last iteration:" << iter << " last residual:" << resNorm
              << " has stagnated:" << stagnated << std::endl;
  }
  // Now with Broyden
    ...
    apsc::make_Jacobian(apsc::JacobianKind::BroydenG));
  {
    ...
    std::cout << std::boolalpha
              << "Broyden (good) Solution has converged: " << converged
              << " last iteration:" << iter << " last residual:" << resNorm
              << " has stagnated:" << stagnated << std::endl;
  }
    ...
    apsc::make_Jacobian(apsc::JacobianKind::EirolaNevanlinna));
  {
    ...
    std::cout << std::boolalpha << "E-N Solution has converged: " << converged
              << " last iteration:" << iter << " last residual:" << resNorm
              << " has stagnated:" << stagnated << std::endl;
  }
}
