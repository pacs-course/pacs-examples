...
{
	std::cout<<std::endl<<this->my_name<<", which is composed by: "<<std::endl;
	for (auto & i : this->my_composite) i->operation();
}
...
{
  for (auto & i : c.my_composite) this->my_composite.emplace_back(i->clone());
}
