...
{
/*!
 * This class is a handle to a sequential random access container (vector or
 * array) of the standard library. It operates directly on the container passed
 * in the constructor and returns with the call operator the sorted container
 * (as side effect) and the permutation old position -> new position after
 * sorting.
 *
 * permutation[i] is the new position in the container of the i-th element
 * before sorting
 *
 * @tparam Container The container
 */
template <class Container> class SortAndPermute
{
public:
   ...
   * passed as argument, and defaulted to less-than, and returns the
   * permutation.
   * @tparam CompOper The type of comparison operator on the container elements
   * (defaulted to less<>)
   * @param compOper  The comparison operator on the container elements.
   * Defaulted to a less<> object
...
SortAndPermute<Container>::operator()(CompOper compOper)
{
