...
//
  ...
  // A trivial mesh with just triangles... 2 triangles meshing a square with vertices numbered 1,2,3,4
  std::vector<Triangle > mesh;

  // this is a trivial mesh used only for example, but the set of operations is the same in complex meshes
  //
  //             4......3
  //              .   /.
  //              .  / .
  ...
  for (auto t: mesh) // loop over mesh triangles
      ...
      for (unsigned int i=0;i<3;++i)
	{
	  ...
	  if (!inserted)  internalMeshEdges.insert(tEdges[i]);
	}
    }
  // Find boundary edges
  std::set<Edge,noOrient> boundaryMeshEdges;
  // I need to give the comparison operator since it is special!
  std::set_difference(allMeshEdges.begin()      ,allMeshEdges.end(),
		      internalMeshEdges.begin(),internalMeshEdges.end(),
		      std::inserter(boundaryMeshEdges,boundaryMeshEdges.begin()),
				    noOrient{});

  std::cout<<" Mesh Information\n N. of Triangles "<<mesh.size()<<std::endl;
  std::cout<<" All Edges:\n";
  for (auto ed : allMeshEdges) std::cout<<ed[0]<<", "<<ed[1]<<std::endl;
  std::cout<<" Internal Edges:\n";
  for (auto ed : internalMeshEdges) std::cout<<ed[0]<<", "<<ed[1]<<std::endl;
  std::cout<<" Boundary Edges:\n";
  for (auto ed : boundaryMeshEdges) std::cout<<ed[0]<<", "<<ed[1]<<std::endl;
