...
main()
{
  ...
  */
  ...
  auto f =
    find_if(dict.begin(), dict.end(), [&word](pair<string, string> const &i) {
  ...
  map<string, string> dict2{
    {"fool", "crazy"}, {"light", "frivolous"}, {"smart", "cool"}};
  ...
  if(node.empty())
    ...
    {
      std::cout << "Extracted node handler with (key, value)= (" << node.key()
                << ", " << node.mapped() << ")\n";
