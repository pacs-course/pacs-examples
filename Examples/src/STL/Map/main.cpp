...
main()
{
  std::map<std::string, std::string> rub;
  ...
  { // I open a scope so I can reuse it and status
    ...
    if(status == false)
      {
        std::cerr << "Marco is already inserted with number " << it->second
                  << std::endl;
      }
  }
  ...
  if(it != rub.end())
    std::cout << "The name corresponding to " << it->second << " is "
              << it->first << std::endl;
  ...
  // the new Marco has not been inserted since already present, but the string
  // il lost;
  if(status == false)
    {
      std::cout << "Marco not inserted\n";
      std::cout << "Number of Marco size is " << numberMarco.size()
                << std::endl;
    }
  ...
  if(status == false)
    {
      std::cout << "Marco not inserted\n";
      std::cout << "Number of Marco size is still " << numberMarco.size()
                << std::endl;
    }
  // This time numberMarco is still alive and kicking.

  // A last utility. Maybe I want to insert or assign a new value:
  // If the key is present give a new value. If not create another entry in the
  // map It can now be done with a single call
  ...
  if(status)
    ...
    {
      std::cout << iter->first << " was already present" << std::endl;
      std::cout << " The number has been changed to " << iter->second
                << std::endl;
    }
