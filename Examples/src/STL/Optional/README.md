...
We recall that an optional is convertible to bool, giving true if a valid value is present. The default constructor sets the optional as invalid. You can set it to a value just by using the assignment operator, while retrieving the value can be done by dereferencing the optional (as it were a pointer) or with the method `value()`. Dereferencing an invalid optional gives undefined behaviour, so always check for valid data before.
