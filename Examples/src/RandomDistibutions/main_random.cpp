...
void distr (Distr & d, Eng & e, const std::string& name,bool file=false, bool hist=true)
{
  ...
  //@note To reduce floating point errors, particularly in the computation of high moments, I order the data
  std::sort(values.begin(),values.end(),[](auto a, auto b){return std::abs(a)<std::abs(b);});
  ...
  for (auto x: values) stat.update(x);
  ...
  if (hist)
    {
      int nBars=25; // numbers of bars in the histogram (if values are floating points)
     ...
     std::cout << "====" << std::endl;
     for (int i=0; i<nBars;++i) {
          std::cout << std::setw(3) << barLegend[i] << ": "
              << valuecounter[i] << std::endl;
      }
#else
      Gnuplot gp;
      std::vector<std::tuple<std::string,double>> gdata;
      for (int i=0; i<nBars;++i) gdata.emplace_back(std::make_tuple(barLegend[i],valuecounter[i]));

      gp<<"set style data histograms\n set style fill solid\n set xtics rotate"<<std::endl;
      gp<<"plot '-' using 2:xtic(1) title '"<<name.c_str()<<"'"<<std::endl;
...
int main()
{
  ...
  std::cout<<"Uniform real distribution in the interval [0,10), Mean=5, Var="<<100/12.<<"Skew= 0, Ex. Kurt.="<<-6./5.<<"\n";
  ...
  std::cout<<"Gamma distribution k=2, theta=1, mean=2, Var=2, Skew="<<2./std::sqrt(2.0)<<", Ex Kurt= 3\n";
  std::gamma_distribution<> gd(2.,1.);
  distr(gd,e,"gamma-distribution-k2-t1",file);

  std::cout<<"LogNormal distribution mu=3, s=2, mean="<<std::exp(5.)<<", Var="<<(std::exp(4.)-1)*std::exp(10)<<
      ", Skew="<<(std::exp(4.0)+2)*std::sqrt(std::exp(4)-1)<<", Ex Kurt"<<std::exp(16.)+2*std::exp(12.)+3*std::exp(8.)-6.0 <<"3\n";
  std::lognormal_distribution<> lnd(3.,2.);
  distr(lnd,e,"lognormal-distribution-m3-s2",file);

  auto sigmaw=std::tgamma(3.)-std::tgamma(2.);
  std::cout<<"Weibull Distribution k=1, l=1, mean="<<std::tgamma(2.0)<<", Var="<<sigmaw*sigmaw
      <<", Skew="<<(std::tgamma(4.0)-3*sigmaw*sigmaw-1)/(sigmaw*sigmaw*sigmaw)<<std::endl;
  ...
  std::cout<<"Student T Distribution nu=2, mean=0, Var=Inf, Skew=Undef, Ex Kurt=Undef";
