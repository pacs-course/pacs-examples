...
//! @return The sampled elements.If n greater than the container size the whole input container is returned
template <class Cont>
Cont sampleContainer(const Cont & in, unsigned int n)
{
  Cont res;
  if (n!=0)
    {
      ...
      else
	{
	  ...
	  // random number generator. I have to use inserter() and not back_inserter,
	  // since it is uniform across containers. This form of calling inserter
	  // is equivalent to back_inserted for std::vector.
	  std::sample(in.begin(), in.end(), std::inserter(res,res.end()),
...
//! @param in The input container. Must have swappable elements. If it can be moved it will.
//! @return a container of the same size, with shuffled elements
template <class Vector>
Vector shuffleContainer(Vector&& in)
{
