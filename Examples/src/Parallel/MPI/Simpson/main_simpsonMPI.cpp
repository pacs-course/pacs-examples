...
int main ()
{
  ...
  if(my_rank==0)
    {
      ...
      double h=(b-a)/n;
      databuf=std::tie(a,b,h,n); // To pack some data (only to show hot it works)
  ...
  auto [a,b,h,n]=databuf; // get global data (in fact we need only h)
  ...
  std::cout<<"Process "<<my_rank<<" a_local, b_local, n_local="<<a_local<<" "<<b_local<<" "<<n_local
      <<std::endl;
  double local_int = SimpsonRule(f,a_local,b_local,n_local);
  double integral{0.};
