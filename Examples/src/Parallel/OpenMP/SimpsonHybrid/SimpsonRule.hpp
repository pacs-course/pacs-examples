...
/*!
 * Computes the simpson composite quadrature rule between a and b using n
 * intervals OpenMP multi-threaded  version
 */
inline double
SimpsonRule_mt(std::function<double(double const &)> const &f, double a,
               double b, unsigned int n, unsigned int num_threads)
{
  double integral{0.};
  double h = (b - a) / n;
