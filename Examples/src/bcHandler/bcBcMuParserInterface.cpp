...
namespace apsc::FEM
{
  ...
  bcMuParserInterface::bcMuParserInterface(const std::string & e): bcMuParserInterface()
      {
    My_e=e;
  ...
  bcMuParserInterface::bcMuParserInterface(bcMuParserInterface const & mpi):
      My_e{mpi.My_e},
      M_parser{static_cast<unsigned int>(mup::pckALL_NON_COMPLEX | mup::pckMATRIX)},
      mup_x{3,0.},
      mup_t{0.}
      {
        M_parser.DefineVar("x",mup::Variable(&mup_x));
      ...
      bcMuParserInterface & bcMuParserInterface::operator = (bcMuParserInterface const & mpi){
        if(this != &mpi)
          {
            this->My_e=mpi.My_e;
  ...
  void bcMuParserInterface::set_expression(const std::string & e)
  {
    M_parser.SetExpr(e.c_str());
  };
