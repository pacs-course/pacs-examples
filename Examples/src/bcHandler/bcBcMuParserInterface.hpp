...
namespace apsc::FEM
{
  ...
  private:
    std::string My_e;
    // static_cast is not needed. Is here only becouse otherwise the stupid eclipse complains!
    mup::ParserX M_parser{static_cast<unsigned int>(mup::pckALL_NON_COMPLEX | mup::pckMATRIX)};
    mup::Value mup_x{3,0.};
    mup::Value mup_t{0.};
  };
