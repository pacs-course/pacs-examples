...
//#include "setbc.hpp"
//
// This main may be replaced to the other main file. It does not meke use
// of the proxy to register boundary conditions.
...
int main(){
	...
	BCContainer allBC ;
	allBC.addBC(BCBase(Dirichlet,std::string("FarField1"),fun1));
	allBC.addBC(BCBase(Neumann,std::string("FarField2"),fun2));
	allBC.addBC(BCBase(Dirichlet,std::string("Wall1"),constant));
	allBC.addBC(BCBase(Dirichlet,std::string("Wall2")));
