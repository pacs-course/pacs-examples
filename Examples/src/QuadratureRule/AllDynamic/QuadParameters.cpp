...
{
QuadParameters
readQuadParameters_GP(std::string const &filename)
{
  using namespace std;
  ...
  if(nlibs == 0)
    {
      throw std::runtime_error(
        "You need to specify at least one plugin library");
    }
  else
    {
  ...
  for(unsigned int i = 0; i < nlibs; ++i)
    {
      ...
      if(quadlib == string("NONE"))
        {
          throw std::runtime_error(
            "Getpot file wrongly parsed,. Cannot read library");
        }
      else
        {
  ...
  if(nlibs == 0)
    {
      throw std::runtime_error(
        "You need to specify at least one integrand library");
    }
  else
    {
  ...
  if(fun_name == "NULL")
    {
      throw std::runtime_error(
        "Getpot file must contain integrand=integrandName\n or integrand=?");
    }
...
QuadParameters
readQuadParameters_json(std::string const &filename)
{
  using namespace std;
  QuadParameters param;
  ...
  // @note: I must tell that the II argument is a std::string, since it is used
  // to derive
  ...
  if(fun_name == "NULL")
    {
      throw std::runtime_error(
        "Json file must contain \"integrand\": integrandName\n or integrand=?");
    }
  param.integrand = std::move(fun_name);
  // The last parameters
