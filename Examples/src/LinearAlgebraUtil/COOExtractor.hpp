...
template <class Value = double> using CooMatrixElements = std::vector<Value>;
...
std::ostream &outputForOctave(std::ostream &               out,
                              CooData<Index, Value> const &coo, int rows,
                              int cols);
...
SparseEigenMatrix make_EigenMatrix(CooData<Index, Value> const &cooMat);
...
template <class Index, class Value>
std::ostream &
outputForOctave(std::ostream &out, CooData<Index, Value> const &coo, int rows,
                int cols)
{
  auto const &values = coo.second;
  auto const &indices = coo.first;
...
make_EigenMatrix(CooData<Index, Value> const &cooMat)
{
  ...
  // Triplet is explicit If the Triplet constructor is not explicit this is not
  // nececcary since the conversion will be implicitely made. An you can just
  // write tripletList.emplace_back(i[0],i[1],values[c++]);
  //
  for(auto i : indices)
    tripletList.emplace_back(EigIndex(i[0]), EigIndex(i[1]),
                             EigValue(values[c++]));
