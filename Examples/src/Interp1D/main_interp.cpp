...
int main()
{
  ...
  // print it out:
    for (auto i: v )std::cout<<i[0]<<" "<<i[1]<<std::endl;
  // interpolate
  for (auto key : keys){
...
// Now the version that takes two vectors separately for keys (nodes) and values

  std::vector<double> nodes;
  std::vector<double> values;
  // fill with same nodes and values as before
  nodes.reserve(v.size());
  for (auto i: v) nodes.emplace_back(i[0]);
  values.reserve(v.size());
  for (auto i: v) values.emplace_back(i[1]);
