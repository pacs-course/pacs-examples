...
int main() {
 ...
 cout.fill(oldfill);
  ...
  cout <<  std::setprecision(maxprec)<<std::scientific<< pi << "Max precision, Scientific format"<<endl;
  cout << std::setprecision(4)<<pi << " with 4 digits, Scientific format"<<endl;
  // I like uppercase E
  ...
  cout<<pi<<" "<<j<<" Internal padding"<<endl;
  cout<<std::setw(25)<<std::left<<pi<<" "<<j<<" Left padding"<<endl;
 //
  cout<<std::showpos<<std::setw(25)<<std::right<<std::setfill('#')<<
    pi<<" "<<j<<" Right padding with # as padding char"<<endl;
  //
  cout << std::showbase<<std::showpos;
  cout <<std::oct<<i << " 1234 in octal base  " <<endl;
  cout << std::hex<<i << " 1234 in exadecimal  "<<endl;
  // Back to normal
  cout<<std::setprecision(oprec)<<std::setfill(oldfill)<<std::setiosflags(oldf);
