...
main()
{
  ...
  if(std::is_trivially_copy_constructible<TrivialCopy>::value)
    ...
    std::cout << "TrivialCopy not trivially copy constructible (cannot "
                 "serialize trivially)\n";
  ...
  if(std::is_trivially_copy_constructible<NoTrivialCopy>::value)
    ...
    std::cout << "NoTrivialCopy not trivially copy constructible (cannot "
                 "serialize trivially)\n";
  binfile.close();
  std::cout << "Created a file of size: "
            << std::filesystem::file_size(datafile) << std::endl;
  ...
  std::cout << " TrivialCopy read. tc1.a=" << tc.getA()
            << ", tc.b=" << tc.getB() << std::endl;
  // close file
  infile.close();
}
