                   ...
                   char* Rhstype)
{
    ...
    * (Title + 72) = (char) NULL;
    ...
    if ( sscanf (line, "%*i%i", Ptrcrd) != 1)
    {
        *Ptrcrd = 0;
    }
    if ( sscanf (line, "%*i%*i%i", Indcrd) != 1)
    {
        *Indcrd = 0;
    }
    if ( sscanf (line, "%*i%*i%*i%i", Valcrd) != 1)
    {
        *Valcrd = 0;
    }
    if ( sscanf (line, "%*i%*i%*i%*i%i", Rhscrd) != 1)
    {
        *Rhscrd = 0;
    }
    ...
    upcase (Type);
    if ( sscanf (line, "%*3c%i", Nrow) != 1)
    {
        *Nrow = 0 ;
    }
    if ( sscanf (line, "%*3c%*i%i", Ncol) != 1)
    {
        *Ncol = 0 ;
    }
    if ( sscanf (line, "%*3c%*i%*i%i", Nnzero) != 1)
    {
        *Nnzero = 0 ;
    }
    if ( sscanf (line, "%*3c%*i%*i%*i%i", &Neltvl) != 1)
    {
        Neltvl = 0 ;
    }
    ...
    * (Rhsfmt + 20) = (char) NULL;
    ...
    if (*Rhscrd != 0 )
    {
        ...
        if ( sscanf (line, "%*3c%i", Nrhs) != 1)
        {
            *Nrhs = 0;
        }
        if ( sscanf (line, "%*3c%*i%i", &Nrhsix) != 1)
        {
            Nrhsix = 0;
        }
    }
    ...
    *rowind = (int*) malloc (*nonzeros * sizeof (int) );
    ...
    if ( Type[0] == 'C' )
    {
        ...
        *val = (double*) malloc (*nonzeros * sizeof (double) * 2);
        if ( *val == NULL )
        {
            IOHBTerminate ("Insufficient memory for val.\n");
        }
    }
    else
    {
                        ...
                        const char* Rhstype)
{
    ...
    if ( Type[0] != 'P' )            /* Skip if pattern only  */
    {
        if ( Valfmt == NULL )
        {
	  Valfmt = const_cast<char *>("(4E20.13)");
        }
        ParseRfmt (Valfmt, &Valperline, &Valwidth, &Valprec, &Valflag);
        if (Valflag == 'D')
        {
                        ...
                        int** rowind, char** val, char** Valfmt)
{
    ...
    *Valfmt = (char*) malloc (21 * sizeof (char) );
    if ( *Valfmt == NULL )
    {
        IOHBTerminate ("Insufficient memory for Valfmt.");
    }
...
int readHB_newaux_char (const char* filename, const char AuxType, char** b, char** Rhsfmt)
{
    ...
    *Rhsfmt = (char*) malloc (21 * sizeof (char) );
    if ( *Rhsfmt == NULL )
    {
        IOHBTerminate ("Insufficient memory for Rhsfmt.");
    }
    readHB_header (in_file, Title, Key, Type, &Nrow, &Ncol, &Nnzero, &Nrhs,
                   Ptrfmt, Indfmt, Valfmt, (*Rhsfmt),
                   &Ptrcrd, &Indcrd, &Valcrd, &Rhscrd, Rhstype);
...
char* substr (const char* S, const int pos, const int len)
{
    ...
    if ( pos + len <= strlen (S) )
    {
        ...
        if ( SubS == NULL )
        {
            IOHBTerminate ("Insufficient memory for SubS.");
        }
        for (i = 0; i < len; i++)
        {
            SubS[i] = S[pos + i];
