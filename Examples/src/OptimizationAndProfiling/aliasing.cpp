...
void
covol(Vet const &A, Vet const &B, Vet &R)
{
 ...
 */
void
covol2(Vet const &A, Vet const &B, Vet &R)
{
  for(unsigned int i = 0; i < A.size(); ++i)
    {
      double tmp = A[i];
...
//! Sums to R the convolution of A and B
//! This version is argument alias proof (and also exception safe)! But it uses
//! more memory Unless you move the arguments
Vet
covol3(Vet A, Vet B, Vet R)
{
  for(unsigned int i = 0; i < A.size(); ++i)
    {
      double tmp = A[i];
