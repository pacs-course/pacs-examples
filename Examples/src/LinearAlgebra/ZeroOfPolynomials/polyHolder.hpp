...
namespace apsc
{
 ...
 /*!
    ...
    *
    * It works internally with complex numbers to be able to compute also complex roots,
    * even if the polynomial has been given by real coefficients.
    *
  ...
  public:
    ...
    //! The container for the coefficients of the polynomials from lowest to highest
     ...
     *
     *  All derivatives at given point and up to given order are computed and stored in the returned vector of
     *  complex numbers. Derivative of order 0 is the value at x.
     *
     *  For Newton-Horner algorithm for the zeroes of a polynomial only the first order derivative is
     *  needed. Nevertheless, since it is not a big problem extending to the case of a generic derivative
     *  I have implemented the general case. First derivative is the default.
     *
          ...
          if (order>1)
            {
              ...
              for  (unsigned i=std::min(order,maxorder)+1;i<=order;++i)
                {
                  values.emplace_back(Coefficient{0.,0.});
                }
            }
   ...
   */
  auto pCoefficients() const {return pCoeff;}
  /*!
    * @return q-Polynomial coefficients (readOnly);
    */
  auto qCoefficients() const {return qCoeff;}
  /*!
   ...
   * @return The coefficients of the n-th order derivative of the stored polynomial
   */
  Coefficients derivativeCoeff(unsigned int n) const
  {
    auto const numCoeff=pCoeff.size();
    if (n>=numCoeff) return Coefficients{{0.,0.}};
  ...
  /*!
     ...
     * Is not a real swap. The new coefficients are the old q's and the old q is cleared
     * Not for general use.
   */
  void swap()
  {
   ...
   *
   * It computes the value of the polynomial at a point and sets qCoeff for the calculation
   * of the derivatives.
   *
   ...
   */
       ...
       *
       * It is a helper function, I put it in the protected section since it is not of general use
       *
       * Returns 1 if p<=n
       * @param p
       * @param n
       ...
       */
      static constexpr unsigned int factor(unsigned int p, unsigned int n=0)
      {
   ...
   *
   * @tparam Coefficients The container for the coefficients of the polynomial Typically a std::vector<complex>
   * @tparam Coefficient The type of the polynomial coefficient (typycally std::complex)
   * @param polyCoefficients The coefficients of the polynomial, in increasing order
   * @param numRoots The number of roots sought (max=the degree of polynomial)
   * @param x0 The initial point for the search.  If you want to find all roots the complex part should be !=0
   ...
   * @return A tuple with the vector of found zeros, the vector of residuals and a status (true=ok, false=failure).
   */
   ...
   for (auto i=0u; i<numRoots;++i)
     {
      ...
      while (!good)
        {
          ...
          good = ((res < tolr) && (std::abs(delta) < tole)) || (iter++ >=maxIter);
        }
      // store the root
      ...
      // Start next search from conjugate (to speed up computation if coefficients are real)
      x=std::conj(x);
     }
