...
namespace apsc
{
   ...
   * @param tol The desired tolerance. It is guaranteed that the found extremum is within the tolerance
  ...
  golden_search(Function const & f, double a, double b, double tol=1.e-5,unsigned maxIter=100)
  {
   ...
   * @return The found bracket points and a status (0=ok, 1= bad initial point, 2 max iteration exceeded)
   *
                  ...
                  unsigned int maxIter = 100)
  {
    ...
    auto y1 = f(x1);
    // Check monotonicity
    if (f(x1-h)< y1 and f(x1+h)>y1) h=-h; // change direction
