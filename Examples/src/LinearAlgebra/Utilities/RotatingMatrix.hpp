...
namespace apsc
{
   ...
   * RotatingMatrix when you exceed the maximum. In particular, if you use NewestReplacesOldest the new column
   * replaces the oldest (faster, but you loose the ordering of the columns). With Shift you shift the columns so that
   * the last inserted is always the last column in the matrix and the order follows the order of insertion.
   */
  enum class InsertStrategy{NewestReplacesOldest, Shift};

  //! This class represents a matrix with N columns that keeps only the latest inserted
  //! columns. It is strongly baded on the eigen
  //!
  //! @tparam T The type stored in the matrix
  //! @tparam M the number of rows
  //! @tparam N the number of columns
template <class T, std::size_t M, std::size_t N, InsertStrategy Strategy=InsertStrategy::Shift>
...
public:
  ...
  auto const &  getEigenFullMatrix()const {return M_vec;}
  //! Returns the underlying container
  auto& getEigenFullMatrix(){return M_vec;}
  //! Returns j-th column as an Eigen Vector
  auto col(std::size_t j)const {return M_vec.col(j);}
  //! Returns j-th column as an Eigen Vector
  decltype(auto) col(std::size_t j){return M_vec.col(j);}
  //! Returns only the active parts of matrix as an Eigen Matrix Block
  //! @pre I must have inserted at least one column
  decltype(auto) getMatrix(){return M_vec.block(0,0,M,N_size);}
  //! Returns only the active parts of matrix as an Eigen Matrix Block
  //! @pre I must have inserted at least one column
  auto getMatrix() const {return M_vec.block(0,0,M,N_size);}
  //! The current inserted number of columns
  auto  cols()const {return N_size;}
  ...
  bool full()const {return N_size==N;}
  //! true if empty
  bool empty()const {return N_size==0u;}
  //! Add a new column
