...
namespace LinearAlgebra
{
  ...
  using
  CooData = std::pair<CooIndices<Index>,CooMatrixElements<Value>>;
  ...
  */
  ...
  template <class Index, class Value>
  std::pair<Index,Index> getRowsAndCols(CooData<Index,Value> const & cooMat)
  {
                                     ...
                                     CooData<Index,Value> const & cooMat)
  {
    ...
    static_assert(std::is_convertible<Index,EigIndex>::value,
                  "Index type in the coo structure is not convertible to the type used by Eigen");
    
    static_assert(std::is_convertible<Value,EigValue>::value,
                  "Value type in the coo structure is not convertible to the type used by Eigen");
    ...
    // Create triplets
    // Note the conversion. Necessary in the case index and values in the coo structure
    // are only convertible to those of eigen and the constructor of Triplet is explicit
    // If the Triplet constructor is not explicit this is not nececcary
    // since the conversion will be implicitely made. An you can just write
    // tripletList.emplace_back(i[0],i[1],values[c++]);
    //
    for (auto i: indices)
      tripletList.emplace_back(EigIndex(i[0]),EigIndex(i[1]),EigValue(values[c++]));
