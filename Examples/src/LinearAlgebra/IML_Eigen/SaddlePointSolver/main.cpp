...
main(int argc, char *argv[])
{
  ...
  if(testParameters.solverSwitch == eigenvalues)
    {
      ...
      Eigen::VectorXd evalues =
        computeSymEigenValues(saddlePointMat.getM(), 2, ncv, maxit, eigtol);
      std::clog << "Eigenvalues of M " << evalues << std::endl;
      std::clog << "Condition Number of M "
                << evalues(0) / evalues(evalues.size() - 1) << std::endl;
      evalues =
        computeSymEigenValues(saddlePointMat.getT(), 2, ncv, maxit, eigtol);
      std::clog << "Eigenvalues of T " << evalues << std::endl;
      auto nn = saddlePointMat.squaredNorm();
      std::clog << "Norm of SPM " << std::sqrt(nn) << " Normalised norm "
                << std::sqrt(nn / saddlePointMat.nonZeros()) << std::endl;
      ...
      // std::clog<<"Eigenvalue of A with smallest
      // magnitude"<<evalues<<std::endl;
      return 0;
    }
  ...
  FVCode3D::PreconditionerFactory factory =
    FVCode3D::make_PreconditionerFactory();
  ...
  if(testParameters.precondSwitch == FVCode3D::DoubleSaddlePoint ||
     testParameters.precondSwitch == FVCode3D::DoubleSaddlePointSym)
    saddlePointMat.convertToDoubleSaddlePoint();
  ...
  switch(testParameters.solverSwitch)
    ...
    case umfpack:
      {
        ...
        std::clog << "size of full matrix:" << A.rows() << " " << A.cols()
                  << std::endl;
    ...
    case gmres:
      {
        ...
        timer.start();
        result =
          GMRES(saddlePointMat, x, b, precond, m, maxit, tol); // Solve system
        timer.stop();
        std::clog << timer;
        std::clog << "End computations\n";
        break;
    ...
    case gmresr:
      {
        ...
        timer.start();
        result =
          GMRESR(saddlePointMat, x, b, precond, m, maxit, tol); // Solve system
        timer.stop();
        std::clog << timer;
        std::clog << "End computations\n";
        break;
    ...
    case fgmres:
      {
        ...
        timer.start();
        auto m = testParameters.gmres_levels;
        result = FGMRES(saddlePointMat, x, b, precond, m, maxit, tol);
        ; // Solve system
        timer.stop();
        std::clog << timer;
        std::clog << "End computations\n";
      ...
      try
        {
          ...
          timer.start();
          result =
            MINRES(saddlePointMat, x, b, precond, maxit, tol); // Solve system
          timer.stop();
          std::clog << timer;
          std::clog << "End computations\n";
        }
    ...
    case tminres:
      std::clog << "Starting computations\n";
      timer.start();
      result = TMINRES(saddlePointMat, x, b, precond, maxit, tol,
                       true); // Solve system
      timer.stop();
      std::clog << timer;
      std::clog << "End computations\n";
