...
//! This utility function computes the Approximate Inverse of the Mimetic Inner
//! Product Matrix
/*!
 * @param Mat a SaddlePointMatrix
 * @param lumping. If true I return the lumped inner-product matrix, if not I
 ...
 * @param D a diagonal matrix that represents an approximate inverse of the
...
//! Computes the approximate Shur complement -T-CA-1Ct using a diagonal
//! approximate inverse for the double saddle point version
/*!
 * @param Mat A saddle point matrix
 * @param D a diagonal matrix that represents an approximate inverse of the
 * inner product matrix
...
//! Computes the -BA-1Bt term using a diagonal approximate inverse for the
//! double saddle point version
/*!
 * @param Mat A saddle point matrix
 * @param D a diagonal matrix that represents an approximate inverse of the
 * inner product matrix
...
//! Computes the CA-1Ct term using a diagonal approximate inverse for the double
//! saddle point version
/*!
 * @param Mat A saddle point matrix
 * @param D a diagonal matrix that represents an approximate inverse of the
 * inner product matrix
 ...
 *
 * @param Mat A saddle point matrix
 * @param lumping If true I uese the lumped inner-product matrix, if not I use
 * the diagonal part.
 * @return
 */
inline SpMat
...
/*!
 * @class preconditioner
 * This is the base class for a preconditioner. As derived classes we can have
 * any preconditioner: identity, diagonal, inexact LU factorization and so on.
 * The goal of these classes is not the assemblation of the preconditioner
 * matrix, but solving the linear system Pz = r in the iterative scheme. They
 * act as a wrapper that allows the practical usage of preconditioner.
 *
 * The preconditioner stores the SaddlePointMat as a pointer. Therefore it
 * implements aggregation, not composition: the SaddlePointMat is an external
 * resource which must be present when the preconditioner is applied.
 *
 * @note I cannot store the SaddlePointMat as reference since the class would
...
public:
  ...
  /*!
   *  If we set lumping=true the approximation of the matrix M in the Shur
   * complement will be made by the lumping strategy, i.e. by summing the rows.
   * The default (lumping=false) uses the diagonal part of M
   */
  void
 ...
 * This class constructs an identity preconditioner. Create and pass an object
 * of this type in the case of running an iterative solver without
 * preconditioning.
 */
 ...
 * This class builds up a ILU preconditioner where the M block is approximated
 * via its diagonal part and the SC is approximated through the inverse of the
 * diagonal part of M. The SC linear system is solved through the Eigen CG.
 *
 */
