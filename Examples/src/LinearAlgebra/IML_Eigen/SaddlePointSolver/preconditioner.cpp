...
ComputeApproximateSchurDSP(const SaddlePointMat &Mat, const DiagMat &D)
{
  ...
  SpMat                                        res =
    -matrix.getBlock({0, 2}) * D * matrix.getBlock({0, 2}).transpose();
...
DoubleSaddlePoint_preconditioner::solve(const Vector &r) const
{
  ...
  z.segment(nVel, nCell) = BAB_chol.solve(
    r.segment(nVel, nCell) + BAmC * z.segment(nVel + nCell, nFrac));
  z.segment(0, nVel) =
    Md_inv * (r.segment(0, nVel) - B.transpose() * z.segment(nVel, nCell) -
              C.transpose() * z.segment(nVel + nCell, nFrac));
  return z;
}
