...
argument are compiled into all translation units that refer to them,
since function template definitions must go in the header file.
...
equal. But unfortunately the linker does not check whether they are!
...
the second is meant to use the second version. However, since three is
not referenced in the function declarations I can avoid including
three.hpp and newthree.hpp in those files.  Indeed, they are included
in the corrresponding source files, fun.cpp and fun2.cpp, where
