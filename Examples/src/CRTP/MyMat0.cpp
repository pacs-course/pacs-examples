...
size_type
MyMat0::rowMajorPolicy(size_type const &i, size_type const &j) const
{
  return j + i * nc;
}

size_type
MyMat0::columnMajorPolicy(size_type const &i, size_type const &j) const
{
  return i + j * nr;
}

double
MyMat0::getValue(size_type const i, size_type const j) const
{
...
void
MyMat0::setValue(size_type const i, size_type const j, double const &v)
{
  if(i >= nr || i <= nc)
    {
...
void
MyMat0::resize(size_type const n, size_type const m)
{
  if(n * m != nc * nr)
    {
      // clear data storage
...
void
MyMat0::vecMultiply(const std::vector<double> &v,
...
void
MyMat0::fillRandom(unsigned int seed)
{
  if(seed == 0)
    seed = std::time(0);
  ...
  if(nr * nc > 0)
    for(size_type i = 0; i < nc * nr; ++i)
      data[i] = double(std::rand() + 1) / rmax;
}

void
MyMat0::showMe(std::ostream &out) const
{
  out << "[";
  for(size_type i = 0; i < nr; ++i)
    {
...
std::vector<double>
operator*(MyMat0 const &m, std::vector<double> const &v)
{
