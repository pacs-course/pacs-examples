...
public:
  ...
  Richardson() = default;
  /*!
   * Provides the function, the number of terms and assumes that  the expansion
  ...
  getError() const
  {
    return std::abs(row0.back() - *(row1.end() - 2u));
  }
  /*!
   ...
   */
  void
  setK(std::vector<unsigned> const &kvalues)
  {
    k = kvalues;
  ...
  /*!
   * Sets the coefficients k of the expansion following a Taylor truncated
   * series
