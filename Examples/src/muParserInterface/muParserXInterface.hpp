...
namespace MuParserInterface
{
  /*! An interface to MuParserX to define a function
    ...
    The input variables are defined as x[0] x[1] etc and one can use the muparserX syntax to create the expression.
    ...
    output are arrays.
    The input variables are indicated by x[]. An example of a valid expression: sin(x[0])+x[1]*x[2]
  ...
  */
  ...
  public:
    //! Default constructor
    //!
    //! mup::pckALL_NON_COMPLEX|mup::pckMATRIX means that I do not want the module for complex numbers
    //! but I want to treat arrays and matrices in muparserX expressions
    muParserXInterface():My_e(),M_parser(mup::pckALL_NON_COMPLEX|mup::pckMATRIX),
                         M_value{N,0.0}
    {
     ...
     *
     * MuparserX has a particular design, which obliges to define a special copy constructor
     * The reson is that a muparser engine stores the address of the variables. So a normal copy
     * would do a shallow copy, which is NOT what you want.
     * Moreover, because of a poor design, you may loose the expression. That's why I keep
     * a copy in the class as a string and a redefine in in the muparser engine.
     *
     * @param mpi the muParserXInterface to be copied
     */
    muParserXInterface(muParserXInterface const & mpi):My_e(mpi.My_e),
