...
public:
  ...
  {
    std::cout
      << " Calling default constructor of Foo with a member vector of size 1000"
      << std::endl;
...
main()
{
  ...
  Foo foo2{foo1};
  std::cout
    << "\n foo3{std::move(foo1)}: Moving the Foo object (foo1 is now emptied)"
    << std::endl;
  ...
  std::cout
    << " Move constructor is called: it is an inizialization NOT an assignement"
    << std::endl;
