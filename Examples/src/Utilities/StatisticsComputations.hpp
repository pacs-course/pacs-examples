...
namespace apsc
{
  namespace Statitics
  {
    ...
    public:
       ...
       * @param x the datum
       */
      void add_variable(double const & x)
      {
        if (n==0u) K = x;
        ++n;
        Ex += x-K;
       ...
       */
      void remove_variable (double const & x)
      {
        if (n==0u) return;
        --n;
        Ex -= x- K;
    ...
    std::array<double,2> shifted_data_mean_variance(Container const & data)
    {
      if (data.size()<2)
        return {data,0.0};
      DataHolder dh;
   ...
   return {dh.get_mean(),dh.get_variance()};
    ...
    /*!
     * Implements Welford recurrence relations for computing statistics incrementally
     */
    ...
    public:
      ...
      void update(const double & x)
      {
        ...
        mean += delta/count;
        M4   += term1*delta_n2*(count*count - 3*count +3.0) + 6.0*delta_n2*M2 - 4.0*delta_n*M3;
       ...
       */
     AggregatedOutput finalize() const
     {
       ...
       return {
         ...
         M2/count,
         M2/(count-1.0),
         std::sqrt(static_cast<double>(count))*M3/std::pow(M2,3./2.),
         (count*M4)/(M2*M2)-3.0
         };
     }
    private:
