 ...
 *
 * This code works only if the arguments are both signed or both unsigned types. Otherwise
 * the situation would be ambiguous since we would perform a comparison between quantities
 * of different type.
 *
 ...
 * @return The absolute difference between a and b. The type is equivalent to decltype(a-b);
 * @pre parameters must be both of signed or unsigned type. Parameters must be subtractable.
 */
