...
namespace apsc
{
  ...
  public:
     ...
     * @param the vectors
     * @note this version does not work if you pass const std::vectors. The reason is that we store pointers in data.
     */
    JoinVectors(std::vector<Types> &... args):
      data(&args...){}
     ...
     */
      void set_Vectors(std::vector<Types> &... args)
      {
        data={&args...};
      }
    //{
    ...
    /*!
     * Extracts the values at the i-th position of the joined vectors (const version)
     ...
     Values operator[](std::size_t i)const
    {
     ...
     /*!
      * Extracts the values at the i-th position of the joined vectors (non-const version)
      * It shows a nic euse of std::apply to generate a tuple of references.
      * @param i The index of the values to be extracted
      * @return A tuple of references to the elements in the joined vector at the i-th position
      */
     decltype(auto)
     operator[](std::size_t i)
     {
       return std::apply([i](auto... x){return std::tie(x->operator[](i)...);},data);
     }
     ...
     auto begin() {return apsc::JoinVectorIterator{*this}.createBegin();};
    //!
     //! @return a const forward iterator representing the end of the joined structure
     auto end() const   {return jVICreator.createEnd();}
     //! @return a non-const forward iterator representing the end of the joined structure
     auto end()   {return apsc::JoinVectorIterator{*this}.createEnd();}
   private:
     /*!
      * An internal funcion to extract the elements at the ith position of th ejoined vector
      * A nice use of recursive function
  ...
  template<class... Types>
    auto join_vectors(std::vector<Types>&... args)
    {
      return JoinVectors<Types...>{args...};
    }
  ...
  template <class... Types>
  bool operator < (const JoinVectorConstIterator<Types...> & a, const JoinVectorConstIterator<Types...> &b){
  ...
  /*!
   ...
   template<class... Types>
     ...
     JoinVectors<Types...> *operator ->() const
     {
       return &myVector;
     }
