//  -*- c++ -*-
//  GetPot Version 1.0.7                                       Jul/10/2005
//
...
GetPot::next(const char* Default)
{
    ...
    std::strncpy(result, Remain.c_str(), Remain.length()+1);
...
GetPot::string_tails(const char* StartString)
{
    ...
    const unsigned            N = static_cast<unsigned int>(std::strlen(StartString));
    ...
    // 0        no argument matches the starting string.
{
    ...
    if( ! search_loop_f ) return static_cast<char *>(0);
