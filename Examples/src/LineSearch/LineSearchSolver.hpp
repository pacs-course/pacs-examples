...
{
  /*!
   * The class that implements a line search technique for unconstrained minimization of a function.
   */
  ...
  public:
     ...
     * @param data The structure containing the cost fuunction and the gradient function
     * @param ddptr a unique pointer to the concrete callable object that computed the search direction
     * @param opt The options for the line search iterations.
     * @param lsOpt The options for the backtracking function.
     * @note The object containing the definition of the desscent direction is MOVED into the class.
     * So you have to be sure that the argument is movable. I have not implemented the clone method for DescentDirectionBase,
     * so this class is NOT copy constructible, not copy-assigneable.
     */
    LinearSearchSolver(
     ...
     * @param descentDirectionFinderPtr The new object that computed the descent direction
     */
    void
  ...
  private:
     ...
     * @return The new point, the new cost value and a status. status=1 -> direction is not a descent direction. status=2 -> number of attempts exceeded
     */
    std::tuple<apsc::LineSearch_traits::Vector, apsc::LineSearch_traits::Scalar, int>
