...
namespace apsc
{
  ...
  struct LineSearchOptions
  {
    ...
    Scalar sufficientDecreaseCoefficient=1.e-2; //!<The coefficient for sufficient decrease
    Scalar stepSizeDecrementFactor=0.5;//!< How much to decrement alpha
    Scalar secondWolfConditionFactor=0.9; //!< Second Wolfe conditin factor (not used)
    Scalar initialStep=1.0; //!< the initial apha value.
    unsigned int maxIter=40;//!< max number of iterations in the backtracking algorithm
  };
