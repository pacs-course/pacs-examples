...
namespace multicity
{
   ...
   *
   * Taylored for the SIS model. It may be easily changed for more complex
   * models
   *
   * @tparam NumCities the number of cities
   */
  template <int NumCities, int NumEpidVariables = 2> struct MultiCityModelTraits
  {
     ...
     */
    static constexpr unsigned int NumEquationsSet =
      NumEpidVariables; // the SIS model has 2
    ...
    using EpidemicVariableType =
      Eigen::Matrix<double, NumEquationsSet * NumCities, NumCities>;
    //! The forcing term type for population model
    using PopulationForcingTermType = std::function<PopulationVariableType(
      const double &, const PopulationVariableType &)>;
    //! The forcing term type for epidemic model
    using EpidemicForcingTermType = std::function<EpidemicVariableType(
      const double &, const EpidemicVariableType &)>;
  ...
  namespace internals
  {
     ...
     */
    template <int NumCities, int Counter, int MaxCounter, typename T,
              typename B, typename... Args>
    void
    join_impl(T &R, B const &add, const Args &... args)
    {
      R.template block<NumCities, NumCities>(Counter * NumCities, 0) = add;
   ...
   * @tparam Args The type matrix blocks to be joined, of dimension of a
   * BlockTye (Numcities x NumCities)
   * @param args The matrices to be joined (in number NumEquationsSet)
   * @return A matrix of type EpidemicVariableType (size
   * NumEquationsSet*NumCities x NumCities)
   */
