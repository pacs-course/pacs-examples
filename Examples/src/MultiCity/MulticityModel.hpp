...
namespace multicity
{
   ...
   */
  template <int NumCities, int NumVariables = 2> struct Result
  {
    using EpidemicVariableType =
      typename MultiCityModelTraits<NumCities,
                                    NumVariables>::EpidemicVariableType;
    using PopulationVariableType =
      typename MultiCityModelTraits<NumCities,
   ...
   */
  template <int NumCities, typename PModel, typename EModel,
            typename RKsolverType = RKFScheme::RK45_t>
  class MultiCityModelAdvance
  {
  public:
    static_assert(PModel::NCity == EModel::NCity,
                  "Population and epidemic model must have same n. of cities");
    /*!
     * Main variables
     *{*/
     ...
     *
     * @todo It can be made copy-constructible, but then I need to set the
     * epidemic data and for that I need a constructor in EpidemicModel taking
     * only the reference to the PopulationVariable it stores as reference!
     */
    ...
    MultiCityModelAdvance(ED<NumCities> const &data)
      : Ncurrent{}, populationModel{data}, epidemicModel{data, Ncurrent},
        initialData{}, RKP{apsc::RKFScheme::make_RK<RKsolverType>(),
                           populationModel},
    ...
    void
    loadEpidemicData(EpiData<NumCities> const &ed)
    {
  ...
  void
  logger(std::ostream &out, RKR const &res)
  {
    out << "Time=" << res.time.back() << " Failed=" << std::boolalpha
        << res.failed;
    out << " Inner iterations=" << res.y.size()
        << " Estimated error= " << res.estimatedError;
  ...
  std::ostream &
  operator<<(std::ostream &out, Result<NumCities> const &res)
  {
    // The format of the file is given by: first the epidemic and then the
    // population
...
template <int NumCities, typename PModel, typename EModel, typename RK>
typename apsc::multicity::Result<NumCities>
apsc::multicity::MultiCityModelAdvance<NumCities, PModel, EModel, RK>::advance()
  const
{
