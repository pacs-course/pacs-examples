...
namespace multicity
{
  template <int NumCities, template <int> class EpiData> class PopulationModel
  {
  public:
    static int constexpr NCity = NumCities;
    using EpidemicDataType = EpiData<NumCities>;
    using VariableType =
      typename MultiCityModelTraits<NumCities>::PopulationVariableType;
    using ForcingTermType =
      typename MultiCityModelTraits<NumCities>::PopulationForcingTermType;
    ...
    VariableType
    operator()(const double &t, const VariableType &N) const
    {
      VariableType F;
      ...
      for(unsigned int i = 0; i < NumCities; ++i)
        {
          for(unsigned int j = 0; j < NumCities; ++j)
            {
              // if (i==j) continue; // skip diagonal (not needed, better let
              // compiler optimize loop)
              F(i, j) +=
                gM.coeff(j, i) * N.coeff(i, i) - d * N.coeff(i, j); // Eq 3.b
            }
          F(i, i) = rNsum(i) + (g(i) * m(i, i) - d) * N.coeff(i, i) +
                    d * Nr(i); // Eq 3.a
        }
      return F;
    }
  ...
  template <int NumCities, template <int> class EpiData,
            class RKsolverType = RKFScheme::RK45_t>
  class advanceMultiCityPopulation
  {
  public:
    ...
    advanceMultiCityPopulation(PopModel const &popModel)
      : model{popModel}, RK{apsc::RKFScheme::make_RK<RKsolverType>(),
                            popModel} {};
