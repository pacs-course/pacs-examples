...
{
  /*!
   * It represents the forcing term in the model SIS presented in the reference
   * article
   * @tparam NumCities N. of cities
   * @tparam EpiData  Edidamic data
   */
  template <int NumCities, template <int> class EpiData> class EpidemicModelSIR
  {
  public:
    ...
    using ForcingTermType =
      typename MultiCityModelTraits<NumCities, 2>::EpidemicForcingTermType;
    ...
    using PopulationVariableType =
      typename MultiCityModelTraits<NumCities, 2>::PopulationVariableType;
    /*}*/
    /*!
     * Constructor
     ...
     */
    EpidemicModelSIR(EpiData<NumCities> const &data, PopulationVariableType &n)
      : data_{data}, N{n} {};
     ...
     */
    void
    setData(const EpiData<NumCities> &data)
    {
      data_ = data;
    }
    /*!
     ...
     *
     * @note An alternative is to aggregate a population variable as a
     * reference. In this way we would eliminate useless copies, but we need to
     * change the constructor.
     *
     * @param n Population variable
     */
    void
    setN(const MultiCityPopulationVariables<NumCities> &n)
    {
      N = n;
    }
  ...
  private:
    ...
    //! Population variable (may become a reference: aggregation instead of
    //! composition)
  ...
  EpidemicModelSIR<NumCities, EpiData>::operator()(const double &      t,
                                                   const VariableType &V) const
  {
    ...
    //*** Block S
    {
      ...
      FS.array() += -d * S.array() +
                    gamma * (1.0 - ifract) * I.array(); // -dS+gamma I term ckd
    ...
    for(int kk = 0; kk < NumCities; ++kk)
      {
        ...
        BlockType  SkN =
          S * k.asDiagonal() * Npi.asDiagonal() * Ik.asDiagonal(); // kSI/Np
