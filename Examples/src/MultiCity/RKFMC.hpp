...
namespace multicity
{
  ...
  template <class VariableType> struct RKFResult
  {
    //! Time steps
    ...
    lastValues() const
    {
      return std::make_tuple(time.back(), y.back());
    }
  };

  template <class B, class Model> class RKFMC
  {
  public:
     ...
     */
    RKFResult<VariableType> operator()(double const &T0, double const &T,
                                       VariableType const &y0,
                                       double const &hInit, double const &tol,
                                       unsigned int maxStep = 2000) const;
  ...
  private:
     ...
     * definition given outside this class template definition. But here it is
     * not necessary. I could have written
     * @code
  ...
  inline std::ostream &operator<<(std::ostream &                 out,
                                  RKFResult<VariableType> const &res);
  ...
  RKFMC<B, Model>::operator()(const double &T0, const double &T,
                              ...
                              const double &tol, unsigned int maxSteps) const
    -> RKFResult<VariableType>
  {
    ...
    unsigned int expectedSteps = std::min(
      std::max(1u, 1u + static_cast<unsigned int>((T - T0) / hInit)), maxSteps);
    ...
    while(t < T && iter <= maxSteps)
      {
        ...
        double mu =
          std::pow(errorPerTimeStep / currentError,
        ...
        if(currentError <= errorPerTimeStep)
          {
            ...
            // Expand next step if error very small, step not previously
            // rejected and I am not at the end
          ...
          {
            rejected = true;
            h *= mu * reductionFactor; // a little more to be sure. Alternative
  ...
  // RKFTraits::VariableType> RKFstep(const double & tstart, const VariableType
  // & y0, const double& h) const
  //
  template <class B, class Model>
  auto
  RKFMC<B, Model>::RKFstep(const double &tstart, const VariableType &y0,
                           const double &h) const
    -> std::pair<VariableType, VariableType>
  {
