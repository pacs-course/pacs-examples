 ...
 */
template <unsigned int N, class R = double> class Polynomial
{
public:
  ...
  template <unsigned int M> Polynomial(Polynomial<M, R> const &right) noexcept
  {
  ...
  auto constexpr
  operator()(R const &x) const noexcept
  {
    //(an*x + an-1)*x + .. + a_0
    auto sum = M_coeff[N];
...
auto
PolyDivide(Polynomial<NDegree, R> const &Num, Polynomial<DDegree, R> const &Den)
  -> std::pair<Polynomial<NDegree - DDegree, R>, Polynomial<DDegree - 1u, R>>
{
  static_assert(
    NDegree >= DDegree,
    "Polynomial Division Error: Numerator degree less than denominator");
  ...
  if(Dend == R{0})
    throw std::runtime_error(
      "Polynomial Division Error. Denominator must be a complete polynomial");
  ...
  while(dN >= DDegree)
    {
      ...
      std::transform(N.begin(), N.end(), d.begin(), N.begin(),
                     [](auto const &n, auto const &d) { return n - d; });
...
/*!
 * Polynomial division. Overload for the division by a constant polynomial
 * (degree=0)
 * @tparam NDegree Degree of the numerator
...
auto
PolyDivide(Polynomial<NDegree, R> const &Num, Polynomial<0u, R> const &Den)
...
template <unsigned int NDegree, typename R>
std::ostream &
operator<<(std::ostream &out, Polynomial<NDegree, R> const &p)
{
...
template <unsigned int NDegree, unsigned int DDegree, typename R>
auto
operator/(Polynomial<NDegree, R> const &Num, Polynomial<DDegree, R> const &Den)
{
          ...
          Polynomial<RDegree, R> const &right) noexcept
{
  ...
  if constexpr(NMAX == LDegree)
    {
      for(unsigned int i = NMIN + 1; i <= NMAX; ++i)
        res.get_coeff()[i] = left.get_coeff()[i];
    }
  else if(NMAX == RDegree)
    {
      for(unsigned int i = NMIN + 1; i <= NMAX; ++i)
        res.get_coeff()[i] = right.get_coeff()[i];
    }
  for(unsigned int i = 0u; i <= NMIN; ++i)
    res.get_coeff()[i] = right.get_coeff()[i] + left.get_coeff()[i];
  return res;
}
          ...
          Polynomial<RDegree, R> const &right) noexcept
{
  ...
  for(unsigned int i = 0u; i <= NMIN; ++i)
    res.get_coeff()[i] = left.get_coeff()[i] - right.get_coeff()[i];
  return res;
}
...
auto
operator*(R const &scalar, Polynomial<RDegree, R> const &right) noexcept
{
...
namespace internals
{
   ...
   */
  template <unsigned int RDegree, typename R, unsigned int Exp> struct Pow
  {
    auto
    operator()(Polynomial<RDegree, R> const &p) const
    {
  ...
  template <unsigned int RDegree, typename R> struct Pow<RDegree, R, 1u>
  {
    Polynomial<RDegree, R>
    operator()(Polynomial<RDegree, R> const &p) const
    {
      return p;
    }
  ...
  template <unsigned int RDegree, typename R> struct Pow<RDegree, R, 0u>
  {
    Polynomial<0u, R>
    operator()(Polynomial<RDegree, R> const &p)
    {
...
template <unsigned int Exp, unsigned int RDegree, typename R>
auto
pow(Polynomial<RDegree, R> const &p)
{
  if constexpr(Exp == 0u)
    return Polynomial<0u, R>{{R(1)}};
