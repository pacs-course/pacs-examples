...
// f(t,y)
double
rk45_step(std::function<double(double const &, double const &)> const &f,
          double const &y0, double const &t0, double const &h, double &error)
{
  ...
  double F5 =
    h * f(t0 + c5 * h, y0 + a51 * F1 + a52 * F2 + a53 * F3 + a54 * F4);
  double F6 = h * f(t0 + c6 * h,
                    y0 + a61 * F1 + a62 * F2 + a63 * F3 + a64 * F4 + a65 * F5);
...
rk45(std::function<double(double const &, double const &)> const &dy,
     double const &t0, double const &T, double const &y0,
     double const &h_initial, double const &h_max, double const &final_error,
     int &status, std::size_t const &maxSteps)
{
  status = 0;
  ...
  std::size_t initialNSteps =
    std::max(static_cast<size_t>(1), static_cast<size_t>(length / h_initial));
  double h = length / initialNSteps;
  // To avoid underflow we need in any case to limit the time step to a positive
  // number Here I allow h to become 128 time smaller than that giving the
  ...
  if(initialNSteps >= maxSteps)
    throw std::runtime_error("RK45: initial time step h too small!");
  ...
  while(time < T && stepsCounter < maxSteps)
    {
      ...
      if(localError > errorPerTimeStep)
        status = 1;
      //! advance
      y = newy;
      time += h;
