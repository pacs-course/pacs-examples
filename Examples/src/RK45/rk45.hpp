...
*/
double
rk45_step(std::function<double(double const &, double const &)> const &dy,
          double const &y0, double const &h, double &error);
...
/*!  Integrates /f[ dy/dt(t)=f(t,y(t)) \quad t \in (t0,T] \quad
  ...
  is given to avoid too large time steps, which may cause overlooking variation
  of f with small support.
...
rk45(std::function<double(double const &, double const &)> const &dy,
     ...
     int &status, std::size_t const &maxSteps = MAX_STEPS);
} // namespace apsc
#endif
