...
# di Milano Copyright Luca Formaggia 2012-2019
...
variable, which should indicate the same directory where this README
file is kept (with full path, i.e. the path starts with /).  You may
change it to adapt the compilation process to your local settings by
editing the file (better choice) or by setting an environmental
variable with the same name (put the command export
PACS_ROOT=myExampleDir in the .profile file in your home directory)

Makefile.inc is included in all othe makefiles and it defines and
...
DOXYFILE Location of the configuration file for DOxygen used in the exercises
CPPFLAGS Flags for the cpp preprocessor
CXXFLAGS Flags for the c++ compiler
DEPEND the file where dependence is autogenerated
...
or by editing the corresponding Makefile (or Makefile.inc if present)
...
Other important environment variables set up by the module system that are not 
used un Makefile.user but may be used by other makefiles of the examples are
