...
read_parameters(GetPot const &ifl)
{
  ...
  r.matrixFileName =
    ifl("matrixFileName", "../../MatrixData/spd/Pres_Poisson.mtx");
...
operator<<(std::ostream &out, TestParameters const &p)
{
  ...
  auto res = std::find_if(getSolver.cbegin(),
                          getSolver.cend(),
...
main(int argc, char *argv[])
{
  ...
  /*
    ...
    D.compute(A);
  */
  // Eigen::IdentityPreconditioner D(A);// Create identity preconditioner (no
  // prec).
