...
void
LinearAlgebra::AdjacencyGraph::info(std::ostream &out, bool verbose) const
{
  ...
  out << "Average row bandwidth: "
      << std::accumulate(bandwidth.begin(), bandwidth.end(), 0) /
  ...
  out << "Average fill ratio   : "
      << std::accumulate(fillRatio.begin(), fillRatio.end(), 0.0) /
           static_cast<double>(fillRatio.size())
      << std::endl;
...
LinearAlgebra::applyRCMK(LinearAlgebra::AdjacencyGraph const &g, bool reverse)
{
  ...
  //! Compare function to order priority queue by node degree with smallest on
  //! top
      ...
      if (R.size() == i)
        {
          ...
          // Take out processed index from the set and get the first unprocessed
          // one
          for (auto &j : s)
            if (!visited[j])
              {
