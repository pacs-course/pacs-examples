...
{
  void 
  dgesv (const int *M, const int *NRHS, const double *A,
         const int *LDA, int *IPIV, double *B, const int *LDB,
         int *INFO);

  void 
  dgetrf (const int *M, const int *N, const double *A, const int *LDA,
          int *IPIV, int *INFO);
      ...
      for (unsigned int jj = 0; jj < get_rows (); ++jj)
        {
          ...
          incrnorm = std::fabs (uh_new - uh(jj, 0)) > incrnorm ?
            std::fabs (uh_new - uh(jj, 0)) :
            incrnorm;
          uh(jj, 0) = uh_new;
        }
      if (incrnorm < tol)
