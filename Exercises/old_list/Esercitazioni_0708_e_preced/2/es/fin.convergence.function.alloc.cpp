/*!
  \file fin.convergence.function.alloc.cpp
...
/*!
  \param vec_ptr address of the vector containing
...
*/
bool solve_GS( std::vector<double>* const vec_ptr,
	       const double dx )
{
  ...
  for( int M = 20; M < MMAX; M *= 2 )
    {
      ...
      vector<double>* uh_ptr( new vector<double>(M + 1) );
	...
	<< "\t" << e << endl;
      ...
      // Free memory (does not destroy uh...)
      delete uh_ptr;
    }
