...
MV_Vector_$TYPE::MV_Vector_$TYPE( int n) : p_(new $TYPE[n]), dim_(n), 
            ref_(0)
{
    if (p_ == NULL)
    {
        std::cerr << "Error: NULL pointer in MV_Vector_$TYPE(int) constructor " << "\n";
...
MV_Vector_$TYPE::MV_Vector_$TYPE( int n, const $TYPE& v) : 
...
{
#ifdef TRACE_VEC
    cout << "> MV_Vector_$TYPE::operator=(const $TYPE & m)  " << "\n";
#endif
...
MV_Vector_$TYPE& MV_Vector_$TYPE::newsize( int n)
{
#ifdef TRACE_VEC
    cout << "> MV_Vector_$TYPE::newsize( int n) " << "\n";
#endif
    ...
    dim_(m.dim_) , ref_(0)
{
    if (p_ == NULL)
    {
        std::cerr << "Error:  Null pointer in MV_Vector_$TYPE(const MV_Vector&); " << "\n";
        exit(1);
    }
...
MV_Vector_$TYPE::MV_Vector_$TYPE($TYPE* d,  int n) : p_(new $TYPE[n]), 
      dim_(n) , ref_(0)
{
    if (p_ == NULL)
    {
        std::cerr << "Error: Null pointer in MV_Vector_$TYPE($TYPE*, int) " << "\n";
        exit(1);
    }
    for (int i=0; i<n; i++)
        p_[i] = d[i];
...
MV_Vector_$TYPE::MV_Vector_$TYPE(const $TYPE* d,  int n) : p_(new $TYPE[n]), 
      dim_(n) , ref_(0)
{
    if (p_ == NULL)
    {
...
std::ostream&   operator<<(std::ostream& s, const MV_Vector_$TYPE& V)
{
    int N = V.size();
