...
namespace LinearAlgebra
{
  ...
  */
  ...
  public:
    ...
    AdjacencyGraph & operator=(AdjacencyGraph const &)=default;
    AdjacencyGraph & operator=(AdjacencyGraph &&)=default;
    ...
    std::vector<Index> ad(Index j) const {return M_graph[j];}
    //! Get degree of a node
    Index degree(Index j) const {return M_degree[j];}
    //! Write info on the graph
    /*!
      \param out The output stream
      \param verbose. If true it prints bandwidth per row.
     */
    void info(std::ostream & out, bool verbose=false) const;
    //! Num Nodes
  ...
  //! sparse matrix expression. Note the use of CRTP, so it can apply efficiently
  //! to any matrix expression, not just a full fledged matrix.
  template<class Derived>
  AdjacencyGraph make_Adjacency(Eigen::SparseMatrixBase<Derived> const & EMat)
  {
  ...
  /*
    ...
    \param V the type of array of permited indices. It should implement begin() and end()
    according to stl rules.
    
    \param EMat The Eigen matrix
    
    \param v The aray of permuted indices i -> v[i]. It is up to the user to verify that they is correct
    in size and stores values corresponding to a permutation. The stored type must be convertible to int.
    
    \return The permuted matrix, with both row and column exchanged
    
    Note: we use a rvalue reference to have a universal reference to the vector. So the vector con be moved
    if neeeded to save memory (not so relevant here, it is for educational purposes).
  ...
  applySymmetricPermutation(EigenM const & EMat, V && v)
  {
    ...
    // I use static_cast becouse I need int, while the indices may have been stored as unsigned int
    // This is to overcome a limitation of Eigen sparse matrices (I hope they fix it soon)
    for (auto const & i: v) indices.coeffRef(j++,0)=static_cast<int>(i);
    EigenM R;
    ...
    // is not working because of limitation in the constructors for Eigen matrices
    return R;
  }

  //! Computes Normal or Reverse Cuthill-McKee ordering 
  std::vector<LinearAlgebra::AdjacencyGraph::Index> applyRCMK(AdjacencyGraph const & g, bool reverse=true);

  //! Applies RCMK algorithm to a eigen matrix
  template<class SpMat>
  SpMat computeRCMK(SpMat const & mat, bool reverse=true)
  {
