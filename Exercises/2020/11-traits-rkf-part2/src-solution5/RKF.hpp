...
template <RKFKind KIND = RKFKind::SCALAR>
...
template <RKFKind KIND>
RKFResult<KIND>
RKF<KIND>::operator()(const VariableType &y0) const
{
  RKFResult<KIND> res;
...
template <RKFKind KIND>
auto
RKF<KIND>::RKFstep(const double &      tstart,
                   const VariableType &y0,
                   const double &      h) const
  -> std::pair<VariableType, VariableType>
{
  unsigned int const &      Nstages{ButcherTable->Nstages};
  std::vector<VariableType> K(Nstages);
  // I use references to simplify typing
  ButcherArray::Atable const &A = ButcherTable->A;
  std::vector<double> const & c{ButcherTable->c};
  std::vector<double> const & b1{ButcherTable->b1};
  std::vector<double> const & b2{ButcherTable->b2};
