  ...
  : Domain(filename)
{
  ...
  n_agents = file((subsection + "n_agents").c_str(), 200);

  n_timesteps_per_day = file((subsection + "n_timesteps_per_day").c_str(), 24);

  n_timesteps = file((subsection + "n_timesteps").c_str(),
                     60 * static_cast<int>(n_timesteps_per_day));
