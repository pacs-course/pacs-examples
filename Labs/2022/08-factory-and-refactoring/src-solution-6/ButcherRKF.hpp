...
#include "../generic-factory/generic_factory.hpp"
...
namespace RKFScheme
{
  ...
  // Factory and proxy class.
  using ButcherFactory = GenericFactory<ButcherArray>;

  template <class RKFDerived>
  class Proxy
  {
  public:
    Proxy(const std::string &name)
    {
      ButcherFactory::register_child<RKFDerived>(name);
    }
  };

  // Register derived classes into factory.
  namespace internal
  {
